...
| 2026-05-15 (Sprint 7 完了) | **V-5 / V-2 / LD-1 を解消**。PR #89 (V-5 Button active scale 0.98 theme override) / #90 (V-2 AgentBar.css に CSS custom properties + light theme override) / #91 (LD-1 LLMSettingsDialog Tabs 化、クラウド/ローカル設定分離)。**V-3 はレビュー結果、現状の `borderRadius: 3` / `4` は pill capsule / LinearProgress の半径 (height/2) として functionally correct なため**コード変更不要としてクローズ。今後 docs/10 §4.6 に "pill 用途" を明示的に追記する案 (future docs update)。残 P2: 他 component CSS (PluginManager.css/SkillRunner.css/UpdateNotification.css) sx 化、KB-3/KB-6 等管理者 UI 詳細点検、C-2/A-2/P-6/P-7 小型 UX 改善。Jest 294/294 維持 |
