...
graph TB
    subgraph "保守太郎 本体 (Host)"
        UI["UI (React)"]
        BE["バックエンド (FastAPI)"]
        GEM["ビルトイン LLM<br/>(ローカル: Gemma 4 / クラウド: AWS Bedrock)"]
...
2. **LLM アダプタ (llm-adapter)**
   ...
   - 保守太郎には標準でビルトイン LLM（ローカルモードは Gemma 4、クラウドモードは AWS Bedrock 経由の Claude）が組み込まれていますが、これを別のAIモデルにすり替えるための標準的な Tool (`generate_text` など) を提供します。
3. **ユーティリティ (utility)**
...
Skill は手順書のような YAML ファイルで、「〇〇コネクタの A という機能を使ってデータを取ってきて、それを使ってから B という機能を呼ぶこと」のようにビルトイン LLM にAIのふるまいを教える設定ファイルです。
