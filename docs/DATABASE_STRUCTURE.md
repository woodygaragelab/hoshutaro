...
| エンティティ | 役割 |
|---|---|
| **WorkOrderClassification** | 作業分類マスター（20種、フラット） |
...
## 1. WorkOrderClassification（作業分類マスター）
...
```typescript
interface WorkOrderClassification {
  id: string;      // "01"-"20"
...
発注/管理単位。Maximo の WorkOrder に相当し、複数の Event × Asset を1つのパッケージとしてグルーピングします。名前は業者への発注名のようなイメージです。
...
| エンティティ | 名前の例 | 意味 |
|---|---|---|
| **WorkOrderClassification** | 年次点検 / オーバーホール | 作業の種類（20種マスター） |
| **WorkOrder.name** | ボイラー設備年次点検整備 | 発注名。複数Event＋機器の集合体名 |
| **WorkOrderLine.name** | ボイラードラム分解清掃 | 具体的な作業。機器部位＋作業名 |
...
## 6. WorkOrderLine（イベントレコード/作業明細）エンティティ

星取表にプロットされる個別のイベントデータ。特定の機器に対する、特定のタイミングでの 1回の作業実行イベント（1 Event × 1 Asset）の計画・実績・費用を管理します。前バージョンにあった `schedule` カレンダー配列をフラット化し、純粋なトランザクションレコードとして扱います。
...
interface WorkOrderLine {
  id: string;
  name: string;                 // → WorkOrderLine.name
  WorkOrderId: string;           // → WorkOrder.id
  AssetId: string;               // → Asset.id
  PlanScheduleStart: Date;   // 計画作業開始日
  PlanScheduleEnd: Date;   // 計画作業終了日
  ActualScheduleStart: Date;   // 実績作業開始日
  ActualScheduleEnd: Date;   // 実績作業終了日
  PlannedManhours?: number;             // 計画作業工数（人時）
  ActualManhours?: number;             // 実績作業工数（人時）
  CreatedAt: Date;
  UpdatedAt: Date;
}
...
```json
{
  "wol-001": {
    ...
    "PlanScheduleEnd": "2024-01-31T00:00:00Z",
    "ActualScheduleStart": "2024-01-01T00:00:00Z",
    "ActualScheduleEnd": "2024-01-31T00:00:00Z",
    "Planned": true,
    "Actual": true,
    "PlanCost": 50000,
    "ActualCost": 48000,
...
```mermaid
erDiagram
    WorkOrderClassification ||--o{ WorkOrder : "1つの作業分類 → N件のWorkOrder"
    ...
    WorkOrderLine {
        ...
        string AssetId FK
        Date PlanScheduleStart
        Date PlanScheduleEnd
    }
...
erDiagram
    WorkOrder {
        ...
        string ClassificationId FK "→ WorkOrderClassification"
        Date CreatedAt "作成日時"
        Date UpdatedAt "更新日時"
    }
    ...
    WorkOrderLine {
        ...
        string name "ボイラードラム分解清掃"
        string WorkOrderId FK "→ WorkOrder"
        string AssetId FK "→ Asset"
        Date PlanScheduleStart "計画開始日時"
        Date ActualScheduleStart "実績開始日時"
        number PlanCost "計画費用"
        number ActualCost "実績費用"
    }
...
1. **WorkOrder → WorkOrderClassification**: 1つのWOは1つの作業分類に属する
2. **WorkOrder → WorkOrderLine**: 1つのWOは複数のLineを持つ（N件の Event × Asset）
3. **WorkOrderLine → Asset**: 各Lineは1つの機器を指す
4. **Asset → HierarchyDefinition**: hierarchyPathの検証
5. **Asset → AssetClassificationDefinition**: classificationPathの検証
...
製油所 > エリア > ユニット > P-101 原油供給ポンプ
  ├─ ボイラー設備年次点検整備 (WorkOrder)           │  ○  │  ●  │     │
  └─ ボイラー設備隔年点検整備 (WorkOrder)           │     │     │  ○  │
```

- **行**: 階層 → Asset → WorkOrder.name（3階層）
- **セル**: 当該Asset×当該WorkOrderにおける、特定年月日の **すべての WorkOrderLine レコードの集約**
- **ダブルクリック**: 当該セルの特定年月日に属する、当該 Asset・WorkOrder に紐づく `WorkOrderLine.name`（個別の作業明細）をダイアログ内に1行1行表示する。
- **コピー＆ペースト操作**: 当該セルに含まれる、機器およびWorkOrderに紐づくすべての `WorkOrderLine` レコードが**丸ごとコピー／ペースト**される。
...
├───────────────────────────────────────────────────────────────┤
│ WO                        │ 作業                │ 計画 │ 実績 │
│ ボイラー設備年次点検整備   │ ボイラードラム分解清掃 (Line)  │  ○   │  ●  │
│ ボイラー設備年次点検整備   │ 弁体摺り合わせ (Line)         │  ○   │     │
│ ボイラー設備年次点検整備   │ 安全弁テスト (Line)           │  ○   │  ●  │
│                                                               │
...
```typescript
type ViewMode = 'asset-based' | 'workorder-based';
```

### 機器ベースビュー行

```typescript
interface AssetBasedRow {
...
interface WorkOrderBasedRow {
  ...
  name?: string;
  ClassificationId?: string;
  // 集約されたスケジュール表示用（特定月内の全関連イベントのサマリ）
...
//   return `${asset.classificationPath['機器種別']}_${wo.ClassificationId}`
// })
```
...
interface WorkOrderLineEditRequest {
  workOrderLineId: string;
  dateKey: string;  // 更新対象のグリッドセル位置
...
2. **WorkOrderLine.AssetId** → Asset.id（必須）
3. **WorkOrder.ClassificationId** → WorkOrderClassification.id（必須）
4. **Asset.hierarchyPath** → HierarchyDefinition.levels（検証）
5. **Asset.classificationPath** → AssetClassificationDefinition.levels（検証）

### 一意性制約

1. **Asset.id**: 全機器で一意
2. **WorkOrder.id**: 全WorkOrderで一意
3. **WorkOrderLine.id**: 全WorkOrderLineで一意

### ビジネスルール

1. **日付プロパティ**: `PlanScheduleStart` 等の各日付項目は ISO 8601形式のDate文字列とする
2. **費用**: 0以上の数値（PlanCost, ActualCost）
3. **工数**: 0以上の数値（PlannedManhours, ActualManhours）
...
interface DataIndexes {
  ...
  workOrderLinesByName: Map<string, WorkOrderLine[]>;
...
|---|---|---|---|
| `WORKTYPE` | `WorkOrderClassification` | ↔ | ほぼ1:1対応 |
...
| `WORKORDER` | `WorkOrder` | ↔ | WONUM → id, WORKTYPE → ClassificationId |
