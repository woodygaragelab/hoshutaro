...
| **Track E: Sprint 0 設計フェーズ** | **完了 (2026-05-16)**。配布形態を Tauri Desktop へ一本化する Track E (~3-4 週間) の Sprint 計画ドキュメント **docs/11_TRACK_E_TAURI_PLAN.md** を新規作成。設計判断 (Tauri 2.x / 既存 React 流用 / Python backend は sidecar 化 / ML 重依存は非同梱で初回 DL / Tauri Updater + minisign / 3 OS 署名) を確定し、Sprint 1-4 ロードマップ + Sprint 1 詳細タスク分解 (Slice 1-A〜1-C) + リスク表を整備。レガシー配布資産 (`launcher/main.py` / `build/build_all.py` / `build/hoshutaro_setup.iss`) を Track E で置換対象として明記 |
...
| Track E: Tauri Desktop（sidecar 化 + 自動更新 + CodeSigning） | Rust、各 OS 証明書、~3-4週間。**Sprint 0 設計完了** (docs/11)、次は Sprint 1 (Tauri スキャフォルド + フロント統合) |
...
2. **Track E: Tauri Desktop** — ~3-4週間。**Sprint 0 設計フェーズ完了** ([docs/11_TRACK_E_TAURI_PLAN.md](docs/11_TRACK_E_TAURI_PLAN.md))。配布形態として一本化必須。**次は Sprint 1 (Tauri スキャフォルド + フロント統合)** — `src-tauri/` 作成 → `package.json` / `vite.config.ts` の Tauri 対応 → `npm run tauri dev` で既存 UI がネイティブウィンドウに表示される状態を作る。Sprint 2 = backend sidecar 化、Sprint 3 = 自動更新 + トレイ、Sprint 4 = コード署名 + リリース CI。
...
5. **Track E: Tauri Desktop** — 上記 #2 参照 (Sprint 0 完了、計画は docs/11)。配布形態として一本化必須。
